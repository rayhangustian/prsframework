...
// Use a placeholder key when none is set so the server can boot fully offline
// (Phase 1 demo). The live /plan /review /synthesize /generate endpoints still
// require a real OPENAI_API_KEY at call time and are otherwise untouched.
...
// Lightweight case list (id + title). Nice-to-have for future case pickers.
app.get('/api/cases', (_req, res) => {
  res.json((cases || [demoCase]).map((c) => ({ id: c.id, title: c.title })));
});

// Returns a pre-rendered synthetic case. If an `id` is supplied (body or query)
// that case is returned; otherwise a RANDOM case is returned. No OpenAI key needed.
...
// Root ('/') is served by express.static as public/index.html.
